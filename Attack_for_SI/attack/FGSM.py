...
class FGSM(Attack):
    ...
    def __init__(self, model, task='CSI', epsilon=0.002, loss='Entropy', targeted=False, 
                batch_size=1, EOT_size=1, EOT_batch_size=1, threshold=0,enroll_embedding=None, confidence=0., 
                verbose=1):
        ...
        self.EOT_wrapper = EOT(self.model, self.loss, self.enroll_embedding, self.threshold, self.EOT_size, self.EOT_batch_size, True)
    ...
    def attack_batch(self, x_batch, y_batch, lower, upper, batch_id, label, attack_transform='', momentum=0.0):
        ...
        for iter in range(self.max_iter + 1):
            ...
            if attack_transform == 'NI':
                x_batch_nes = x_batch.data - self.step_size * momentum * grad_before
            else:
                x_batch_nes = x_batch
            scores, loss, grad, decisions = self.EOT_wrapper(x_batch_nes, y_batch, EOT_num_batches, real_EOT_batch_size, use_grad)
            scores.data = scores / EOT_num_batches
            loss.data = loss / EOT_num_batches
            grad = torch.where(grad==0, torch.zeros_like(grad) + 1e-8, grad)
            ...
            if iter < self.max_iter:
                if attack_transform == 'MI' or attack_transform == 'NI':
                    k = grad / torch.abs(grad)
                    k = momentum * grad_before + k
                    grad_before = k.data.detach() + 0 
                    grad = k
    ...
    def attack(self, x, y, label):
        ...
        lower = -1*self.epsilon + x
        upper = self.epsilon + x
        ...
        for batch_id in range(n_batches):
            ...
            adver_x_batch, success_batch = self.attack_batch(x_batch, y_batch, lower_batch, upper_batch, batch_id, label)
            if batch_id == 0:
