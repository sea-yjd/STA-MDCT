...
class PGD(FGSM):
    ...
    def __init__(self, model, task='CSI', epsilon=0.002, step_size=0.0004, max_iter=10, num_random_init=0, 
                ...
                batch_size=1, EOT_size=1, EOT_batch_size=1, threshold=0, target=2,
                enroll_embedding=None, confidence=0., attack_transform='I', momentum=1.0,
                verbose=1):
    ...
    def attack(self, x, y, label):
        ...
        for init in range(max(1, self.num_random_init)):
            ...
            for batch_id in range(n_batches):
                ...
                adver_x_batch, success_batch = self.attack_batch(x_batch, y_batch, lower_batch, upper_batch, '{}-{}'.format(init, batch_id), label, self.attack_transform, self.momentum)
                if batch_id == 0:
