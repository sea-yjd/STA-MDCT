...
class Model(LightningModule):
    ...
    def get_farthest_target_id(self, x, enroll_embedding, label):
        scores = None
        test_embedding = self.extract_speaker_embedding(x).squeeze(0)
        # scores = torch.zeros((x.shape[0], len(enroll_embedding.enroll_data)), device=test_embedding.device)
        for i, enroll_embed in enumerate(enroll_embedding.enroll_data):
            ...
            if scores == None:
                ...
                scores = torch.cat((scores, score), dim=0)
        # scores = scores.reshape((1,-1))
        min_value, min_idx = torch.min(scores, dim=0)
        # scores[max_idx] = float('-inf')
        # second_max_value, second_max_idx = torch.max(scores, dim=0)

        return min_idx.unsqueeze(0)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--nnet_type', type=str, default="RawNet3")  # ['ECAPATDNN', 'ResNetSE34L', 'RawNet3', 'ResNetSE34V2']
