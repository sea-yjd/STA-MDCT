...
import numpy as np
from .compute_eer import compute_eer, compute_mindcf, calculate_fpr_at_frr
    ...
    with open(score_file, "w") as f:
        for item in trials:
            ...
            if apply_length_norm:
                enroll_vector = length_norm(enroll_vector)
                test_vector = length_norm(test_vector)
                # enroll_vector = F.normalize(torch.tensor(enroll_vector).cuda(), p=2, dim=1)
                # test_vector = F.normalize(torch.tensor(test_vector).cuda(), p=2, dim=1)
            score = enroll_vector.dot(test_vector.T)
            # score = F.cosine_similarity(enroll_vector, test_vector, dim=1)
            score = np.mean(score)
            denom = np.linalg.norm(enroll_vector) * np.linalg.norm(test_vector)
            score = score/denom
            ...
            if item[0] == "1":
                ...
                nontarget_scores.append(score)
            all_label.append(int(item[0]))
            all_scores.append(score.tolist())
    eer, th = compute_eer(target_scores, nontarget_scores)
    mindcf = compute_mindcf(all_scores, all_label)
    return eer, th, mindcf
