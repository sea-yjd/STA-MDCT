...
from .backend import compute_eer, cosine_score, PLDA_score, save_cosine_score, compute_mindcf
...
class Model(LightningModule):
    def __init__(self, **kwargs):
        ...
        # 2. Speaker_Encoder
        if kwargs['model_num'] > 1:
            nnet_name = kwargs['checkpoint_path'].split('/')[-1].split('.')[0]
            Speaker_Encoder = importlib.import_module('speaker_trainer.nnet.'+ nnet_name).__getattribute__('MainModel')
        else:
    ...
    def extract_speaker_embedding(self, data):
        ...
        if self.hparams.model_num > 1:
            nnet_name = self.hparams.checkpoint_path.split('/')[-1].split('.')[0]
            if nnet_name == 'ECAPATDNN':
    ...
    def cosine_evaluate(self, trials=None):
        ...
        print("scoring...")
        score_file = self.hparams.trials_path[:-4] + "_victim_" + self.hparams.nnet_type + "adt_score.lst"  # TODO
        eer, th, mindcf = save_cosine_score(trials, index_mapping, eval_vectors, score_file)
        print("Cosine EER: {:.3f}%".format(eer*100))
        print("threshold: {:.3f}".format(th))
        print('mindcf: {:.3f}'.format(mindcf))
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--nnet_type', type=str, default="ECAPATDNN", choices=['ECAPATDNN', 'ResNetSE34L', 'RawNet3', 'ResNetSE34V2', 'BRANCH_ECAPA_TDNN'])
        parser.add_argument('--config', type=str, default='')
        parser.add_argument('--pooling_type', type=str, default="SAP", choices=['SAP', 'ASP', 'ECA'])
        parser.add_argument('--threshold', type=float, default=0)
        parser.add_argument('--augment', action='store_true', default=False)
        parser.add_argument('--max_frames', type=int, default=400)
        parser.add_argument('--eval_frames', type=int, default=400)   #set max_frames=0 for evaluation. If max_frames=0, then the returned feat is a whole utterance.
        parser.add_argument('--num_eval', type=int, default=10)
        parser.add_argument('--min_frames', type=int, default=200)
        parser.add_argument('--n_mels', type=int, default=80)
        parser.add_argument('--sinc_stride', type=int, default=0)
        parser.add_argument('--train_list_path', type=str, default='')
        parser.add_argument('--trials_path', type=str, default='data/trials.lst')   ## 'your_data_path/trials.lst'
        ...
        parser.add_argument('--checkpoint_path', type=str, default='/home/jdyao/PycharmProjects/spot-adv-by-vocoder-main/speaker_verification/pretrained_model/RawNet3_model.pt')
